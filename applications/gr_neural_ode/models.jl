...
function AbstractNROrbitModel1(u, model_params, t;
                              NN_chiphi=nothing, NN_chiphi_params=nothing,
                              NN_pe=nothing, NN_pe_params=nothing)
    #=
        Defines system of odes which describes motion of
    ...
    numer = (1+e*cos(χ))^2
    denom = M*(abs(exp(logp))^(3/2))

    χ̇ = (numer / denom) * nn_chiphi[1]
    ϕ̇ = (numer / denom) * nn_chiphi[2]
    # chain rule for time derivative dp/dt = d/dt(exp(logp))
    logpdot = nn_pe[1]/logp
    ė = nn_pe[2]

    return [χ̇, ϕ̇, logpdot, ė]
end
